#$say dnum:$(dnum) ddec:$(ddec) ws:$(ws) dir:$(dir)
...
$execute positioned ^ ^ ^$(dnum).$(ddec) if block ~ ~ ~ minecraft:$(ws) store result score @e[sort=nearest,type=minecraft:marker,tag=su.ws,distance=..0.1] su.wsY run data get entity @s Rotation[0]
$execute positioned ^ ^ ^$(dnum).$(ddec) if block ~ ~ ~ minecraft:$(ws) store result score @e[sort=nearest,type=minecraft:marker,tag=su.ws,distance=..0.1] su.wsX run data get entity @s Rotation[1]
