...
scoreboard players set @e[sort=nearest,type=armor_stand,tag=slutchest] slutTest 0
execute store success score @s slutTestSpawn run forceload add ~ ~ ~ ~
setblock ~ ~ ~ chest[facing=south,type=right]{CustomName:'{"bold":true,"color":"dark_purple","italic":false,"text":"Item Overflow"}'}
setblock ~1 ~ ~ chest[facing=south,type=left]{CustomName:'{"bold":true,"color":"dark_purple","italic":false,"text":"Item Overflow"}'}
execute as @e[sort=nearest,type=minecraft:armor_stand,tag=slutchest] at @s run tp @s ~0.5 ~1 ~
