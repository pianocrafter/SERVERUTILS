...
execute if data storage serverutil:temp HeadItem at @s run summon minecraft:item ~ ~ ~ {Tags:["hatSavedItem"],Item:{id:"minecraft:stone",Count:1b}}
execute if data storage serverutil:temp HeadItem at @s run data modify entity @e[limit=1,sort=nearest,type=item,tag=hatSavedItem] Item set from storage serverutil:temp HeadItem
